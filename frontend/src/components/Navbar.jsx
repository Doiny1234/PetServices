...
import { FiSearch, FiUser, FiX, FiMenu, FiLogOut } from "react-icons/fi";
import { IoMdArrowDropdown } from "react-icons/io";
import { Link, useNavigate } from "react-router-dom";

const Navbar = () => {
  // Stări pentru gestionarea vizibilității diferitelor elemente din navbar
  ...
  const token = localStorage.getItem("token"); // Preluăm token-ul pentru a verifica dacă utilizatorul este logat
  ...
  // Funcție pentru a gestiona căutarea la apăsarea tastei Enter
  const handleSearch = (e) => {
    if (e.key === "Enter") {
      if (searchTerm.trim()) {
        // Navighează către pagina de servicii cu parametrul categoriei
  ...
  // Funcție pentru a deschide/închide meniul pe mobil
  ...
  return (
      <header className="bg-white shadow-md px-6 py-4 lg:px-12 sticky top-0 z-50">
        <div className="flex justify-between items-center">
          ...
          {/* Meniul pentru vizualizarea de tip Desktop */}
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-[#212e0e]">
            ...
            {/* Meniul Dropdown pentru Servicii */}
            <div
                className="relative"
            ...
            >
              ...
              {/* Opțiunile din meniul dropdown de servicii */}
              {showDropDown && (
          ...
          {/* Iconițe și bara de căutare pentru Desktop */}
          <div className="hidden md:flex items-center gap-3 relative">

            {/* Câmpul de introducere text pentru căutare */}
            ...
            {/* Iconița pentru utilizator (redirecționează către dashboard sau login) */}
            <Link
            ...
            {/* Buton de logout vizibil exclusiv dacă utilizatorul este logat */}
            {token && (
                <button
          ...
          {/* Butoanele de control pentru vizualizarea de tip Mobil */}
        ...
        {/* Meniul propriu-zis pentru varianta de Mobil */}
        <div
            className={`${
        ...
        >
          ...
          {/* Buton de logout direct din meniul de mobil */}
          {token && (
              <button
                  onClick={handleLogout}
